...
export default function EleamFilters({ filters, setFilters, count }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 mb-4 grid grid-cols-1 md:grid-cols-5 gap-3">
      <div className="md:col-span-2">
        <label className="text-xs uppercase tracking-wide text-gray-500 font-semibold mb-1 block">
          Buscar
        </label>
        <input
          type="search"
          ...
          className="w-full border border-gray-300 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-slate-400"
        />
      </div>
      <div>
        <label className="text-xs uppercase tracking-wide text-gray-500 font-semibold mb-1 block">
      ...
      <div className="md:col-span-5 flex items-center justify-between flex-wrap gap-2 pt-1">
        <span className="text-xs text-gray-500">{count} ELEAM(s) coinciden</span>
        <button
          type="button"
          onClick={() => setFilters({})}
