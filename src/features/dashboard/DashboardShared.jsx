...
export function KpiCard({ title, value, sub, icon, tone = "primary", onClick, help }) {
  ...
  return (
    <article className={`group text-left ${t.bg} rounded-2xl border border-slate-100 shadow-sm p-5 hover:shadow-md hover:-translate-y-0.5 transition-all`}>
      <div className="flex items-start justify-between">
        <span className="text-xs uppercase tracking-wide text-slate-400 font-medium inline-flex items-center gap-1.5">
          <span>{title}</span>
          {help && <HelpTooltip label={`Ayuda: ${title}`}>{help}</HelpTooltip>}
        </span>
        <span className={`h-8 w-8 rounded-xl flex items-center justify-center ${t.chip}`}>
...
export function AlertChip({ label, value, tone, onClick, hint }) {
  ...
  return (
    ...
    >
      ...
      <div className="text-xs text-slate-500 line-clamp-1">{hint}</div>
    </button>
  );
}
...
export function BriefMetric({ label, value, status, sub, tone, help }) {
  const toneClass = {
    primary: { value: "text-teal-700 bg-teal-50", status: "bg-teal-50 text-teal-700 border-teal-100" },
...
export function MiniVitals({ s }) {
  const Pill = ({ label, value }) =>
    value ? (
      <span className="hidden md:inline-flex items-center gap-1 text-[10px] text-slate-500 bg-slate-50 border border-slate-100 rounded px-1.5 py-0.5 tabular-nums">
