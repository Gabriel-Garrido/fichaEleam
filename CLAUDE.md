...
npm run build     # /dist + SEO/LLM prerender para cPanel
npm run lint      # ESLint
...
└── utils/
    ...
    ├── passwordValidation.js   # validatePassword(password, confirm) — compartido por ResetPassword y ChangePasswordPage
...
6. **Acceso Gmail pendiente**: `create-staff-user` deja un acceso pendiente para correos Gmail y `handle_new_user` lo consume cuando el usuario entra con Google. El registro público por token fue retirado; `/register` redirige a `/login`.

**Regla central**: no existe auto-registro público. `handle_new_user` solo acepta superadmin plataforma, demo aprobada por superadmin, usuario creado por Edge Function con `app_metadata` de Admin API, o acceso Gmail pendiente creado por el ELEAM. `raw_user_meta_data` nunca se usa para autorizar `rol` ni `eleam_id`.
...
| Plan | precio_clp | max_residentes | max_funcionarios | Notas UI |
|------|-----------|---------------:|-----------------:|----------|
| `plan-14` | 50.000 | 14 | 10 | "$50.000 / mes + IVA" — `~$1.700/día` |
| `plan-24` | 80.000 | 24 | 20 | "$80.000 / mes + IVA" — `~$2.700/día`, etiqueta "Más elegido" |
| `plan-34` | 120.000 | 34 | 30 | "$120.000 / mes + IVA" — `~$4.000/día` |
| Institucional | — | 35+ | A medida | "Cotización personalizada" — CTA abre WhatsApp (`source="institutional"`) con mensaje pre-cargado "35+ residentes" |

Precios netos (sin IVA, B2B chileno) tanto en UI como en JSON-LD (`UnitPriceSpecification.valueAddedTaxIncluded: false`). Cambiar `WHATSAPP_PHONE` o el tier institucional en `src/features/landing/whatsAppLeadUtils.js` y en la tarjeta institucional dentro de `LandingPage.jsx`. El catálogo público vive en `src/features/payment/planCatalog.js` y debe mantenerse sincronizado con el seed `public.planes` en `supabase_schema.sql`.

Reglas de cupo productivas:

- Residentes que consumen cupo: `activo` + `hospitalizado`.
- Residentes que no consumen cupo: `egresado` + `fallecido`.
- Funcionarios consumen cupo si existen en `profiles` o si tienen invitación Gmail pendiente en `funcionario_invitaciones`.
- Familiares no consumen cupo de funcionarios.
- La UI bloquea crear/importar/activar sobre cupo; Supabase vuelve a validar con `check_residentes_limit`, `check_funcionarios_limit`, `check_eleam_plan_capacity`, `mp-create-subscription` y `registrar_pago_y_activar_eleam`.
...
**SEO / LLM**: Meta tags (description, robots `max-image-preview:large`, OG con `og:locale`/`og:site_name`, Twitter `summary_large_image`), JSON-LD (Article+BlogPosting con ImageObject/wordCount/articleSection/isPartOf/speakable, BreadcrumbList por post, Organization, SoftwareApplication, FAQPage, HowTo, Blog schema con publisher), `robots.txt` (GPTBot, ClaudeBot, PerplexityBot, etc.), `sitemap.xml` (con imágenes) y `llms.txt`. `npm run build` ejecuta `scripts/generate-public-seo.mjs`, que **lee `supabase_blog_seed.sql` como fuente de verdad** (los seeds del blog son permanentes) y genera en `dist` HTML prerenderizado de `/`, `/blog`, `/pago`, páginas públicas y cada post publicado. Cada post prerenderizado incluye breadcrumb, etiquetas, `<time>`, CTA con enlaces internos (demo, software, acreditación, pago, WhatsApp) y bloque de artículos relacionados (calculado por keywords compartidas) para potenciar el enlazado interno y la indexación. Genera además `.htaccess` (cPanel/HostGator) y dominio canónico `https://fichaeleam.cl`. Los archivos `public/{sitemap.xml,robots.txt,llms.txt}` son placeholders; el build los reemplaza en `dist`.

**Convención de URL canónica (trailing slash)**: el host sirve cada ruta pública como directorio (`dist/<ruta>/index.html`), por lo que la URL real lleva slash final y Apache redirige 301 de `/ruta` a `/ruta/`. Por eso **canonical, `og:url`, `<loc>` del sitemap, los `@id`/`url` de JSON-LD y los enlaces internos del prerender usan la forma con slash** (`/software-eleam/`, `/blog/<slug>/`). Helpers `withTrailingSlash`/`routeUrl` en `scripts/generate-public-seo.mjs` y en `src/utils/seo.js` (este último normaliza el `path` de `useSEO`, que de lo contrario sobrescribiría el canonical con la forma sin slash al hidratar). Mantener ambas fuentes alineadas evita "Rastreada: actualmente sin indexar / Página con redirección" en Search Console.
